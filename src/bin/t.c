...
int main(int argc, char *argv[])
{
	...
	if(argc != 11) // 1:wdir 2:cube.fits (m/s) 3:ref.vf.fits (km/s) 4:xlower 5:ylower 6:xupper 7:yupper
	{
		usage_baygaud();
		exit(0);
	}
...
*/
    ...
    else// sub-nodes with ranks
    {
		...
		// do analysis
		//ext_bulk_motions(process_X[rank][0], process_X[rank][1], process_Y[rank][0], process_Y[rank][1], multinest_param, TRparam, sorting_order, raw_cube_mpi[0].data, rank);
		int thread_status_slave=0; // should be declared
		int _t=0;
		ThreadArgs *thread_args;
		thread_args = (ThreadArgs *)malloc(sizeof(ThreadArgs));
		thread_args->process_X0 = process_X[rank][0];
...
opt_gauss_number = gfit_params_mpi[0].data[process_X[0][0]][process_Y[0][0]][0][0][5];
printf("%d %d %f\n", process_X[0][0], process_Y[0][0], gfit_params_mpi[0].data[process_X[0][0]][process_Y[0][0]][opt_gauss_number-1][0][3]);

opt_gauss_number = gfit_params_mpi[0].data[process_X[1][0]][process_Y[1][0]][0][0][5];
printf("%d %d %f\n", process_X[1][0], process_Y[1][0], gfit_params_mpi[0].data[process_X[1][0]][process_Y[1][0]][opt_gauss_number-1][0][3]);
		for(n=0; n<n_node; n++)
		{
			i0 = process_X[n][0];
			j0 = process_Y[n][0];
