		...
		//	printf("x:%4d y:%4d being processed...", i, j);
			...
			if(!isnan(HI_cube_mpi[i][j][0]) && !isinf(HI_cube_mpi[i][j][nax3-1]))
			{
				...
				//for(_n0=4; _n0<5; _n0++)
				{
					...
					*/
					//printf("%d %d thread_normal_exit:%d\n", i, j, thread_normal_exit);
					//if(thread_normal_exit == 0) printf("exit normally: ");
					//if(thread_normal_exit == 1) printf("exit ab-normally: ");
					//pthread_join(p_thread_gfit, (void **)&thread_status); // my_thread_args->status, thread_normal_exit values are meaningless after pthread_join
					//Gfit_multinest_student(multinest_param, TRparam); // normal multinest without the thread option

					if(thread_normal_exit != 1 || _n0 == 1) // normal exit or single gaussian fit
					{
						//printf("%d (%d %d : rank-%d : Gaussians: %d)\n", thread_normal_exit, i, j, rank, TRparam[0].n_gauss);
...
//printf("here %f\n", gfit_params_mpi[0].data[i][j][0][0][5]);
					}
...
gfit_params_mpi[0].data[i][j][(int)gfit_params_mpi[0].data[i][j][0][0][5]-1][1][1]/(sqrt(2*M_PI)*gfit_params_mpi[0].data[i][j][(int)gfit_params_mpi[0].data[i][j][0][0][5]-1][1][2]));
			...
			process_time_expected = (((xupper-xlower)*(yupper-ylower) - n_profiles_analysed) * (elapsed / n_profiles_analysed)) / 60.; // in minutes
	
			if((int)n_profiles_analysed % 50 == 0 && n_profiles_analysed != (xupper-xlower)*(yupper-ylower))
			{
				printf("| rank-%4d : x[%5d%5d] y[%5d%5d] : %10.1f %% <- %10.1f sec | -> %10.1f min remains |\n", rank, xlower, xupper, ylower, yupper, 100*n_profiles_analysed/((xupper-xlower)*(yupper-ylower)), elapsed, process_time_expected);
			}

			if(n_profiles_analysed == (xupper-xlower)*(yupper-ylower))
			{
				printf("| rank-%4d : x[%5d%5d] y[%5d%5d] : %10.1f %% <- %10.1f sec | ->             [completed] |\n", rank, xlower, xupper, ylower, yupper, 100*n_profiles_analysed/((xupper-xlower)*(yupper-ylower)), elapsed);
			}
		}
