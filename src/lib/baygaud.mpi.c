...
MPI_Datatype allocate_mpi_dataset(TR_ringParameters *TRparam, MPI_Datatype *type, int *blocklen)
{
    ...
    // 303. nax3 
    MPI_Get_address(&TRparam[0].nax3, &address2);
    disp[302] = address2 - address1;

    // 304. sorting_order 
    MPI_Get_address(&TRparam[0].sorting_order, &address2);
    disp[303] = address2 - address1;
    // 305. N_channels 
    MPI_Get_address(&TRparam[0].N_channels, &address2);
    disp[304] = address2 - address1;
    // 306. N_channel_outliers 
    MPI_Get_address(&TRparam[0].N_channel_outliers, &address2);
    disp[305] = address2 - address1;
    // 307. outliers_channelNumber 
    MPI_Get_address(&TRparam[0].outliers_channelNumber, &address2);
    disp[306] = address2 - address1;
    // 308. velocity_profile_norm 
    MPI_Get_address(&TRparam[0].velocity_profile_norm, &address2);
    disp[307] = address2 - address1;
    // 309. velocity_profile 
    MPI_Get_address(&TRparam[0].velocity_profile, &address2);
    disp[308] = address2 - address1;
    // 310. velocity_profile_e 
    MPI_Get_address(&TRparam[0].velocity_profile_e, &address2);
    disp[309] = address2 - address1;
    // 311. velocity_offset 
    MPI_Get_address(&TRparam[0].velocity_offset, &address2);
    disp[310] = address2 - address1;
    // 312. Velocity_Seperation 
    MPI_Get_address(&TRparam[0].Velocity_Seperation, &address2);
    disp[311] = address2 - address1;
    // 313. Reference_Velocity 
    MPI_Get_address(&TRparam[0].Reference_Velocity, &address2);
    disp[312] = address2 - address1;
    // 314. vel_profile_res_RBM 
    MPI_Get_address(&TRparam[0].vel_profile_res_RBM, &address2);
    disp[313] = address2 - address1;
    // 315. vel_profile_res_SIGMA 
    MPI_Get_address(&TRparam[0].vel_profile_res_SIGMA, &address2);
    disp[314] = address2 - address1;
    // 316. effective_channel_start 
    MPI_Get_address(&TRparam[0].effective_channel_start, &address2);
    disp[315] = address2 - address1;
    // 317. effective_channel_end 
    MPI_Get_address(&TRparam[0].effective_channel_end, &address2);
    disp[316] = address2 - address1;
    // 318. unit_channel_width 
    MPI_Get_address(&TRparam[0].unit_channel_width, &address2);
    disp[317] = address2 - address1;

    // 319. xlower 
    MPI_Get_address(&TRparam[0].xlower, &address2);
    disp[318] = address2 - address1;
    // 320. xupper 
    MPI_Get_address(&TRparam[0].xupper, &address2);
    disp[319] = address2 - address1;
    // 321. ylower 
    MPI_Get_address(&TRparam[0].ylower, &address2);
    disp[320] = address2 - address1;
    // 322. yupper 
    MPI_Get_address(&TRparam[0].yupper, &address2);
    disp[321] = address2 - address1;
    // 323. _i_ 
    MPI_Get_address(&TRparam[0]._i_, &address2);
    disp[322] = address2 - address1;
    // 324. _j_ 
    MPI_Get_address(&TRparam[0]._j_, &address2);
    disp[323] = address2 - address1;


    //MPI_Datatype TRparam_mpi;
    MPI_Type_create_struct(324, blocklen, disp, type, &TRparam_mpi);
    return TRparam_mpi;
