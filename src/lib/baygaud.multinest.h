       ...
       #define NESTRUN_MPIOFF __nested_mpioff_MOD_nestrun_mpioff
#else
...
// Multiple Gaussian fit
    int sorting_order; // channel order, decreasing or increasing?
    int N_channels; // number of channels
    int N_channel_outliers; // number of outlier channels 
    int outliers_channelNumber[999]; // outlier channel number
    double velocity_profile_norm[999]; // normalised velocity profile : number of channels
    double velocity_profile[999]; // velocity profile : number of channels
    double velocity_profile_e[999]; // velocity profile error: number of channels
// 310

    // channel information
    float velocity_offset;
    float Velocity_Seperation;
    float Reference_Velocity;
    double vel_profile_res_RBM;
    double vel_profile_res_SIGMA;
    int effective_channel_start;
    int effective_channel_end;
    double unit_channel_width;
// 318

    /* Set velocity field area */
    int xlower;
    int xupper;
    int ylower;
    int yupper;
// 322
	int _i_; // pixel i : made for multi-dimensional g_param[_i_][_j_][99]
	int _j_; // pixel j
// 324
} TR_ringParameters;
...
extern void NESTRUN_MPIOFF(int *, int *, int *, int *, double *, double *, int *, int *, int *, int *, int *, double *, 
char *, int *, int *, int *, int *, int *, int *, double *, int *, void (*Loglike)(double *, int *, int *, 
double *, TR_ringParameters *), void (*dumper)(int *, int *, int *, double **, double **, double **, double *, 
double *, double *, double *, TR_ringParameters *), TR_ringParameters *context);
...
void run_mpioff(int IS, int mmodal, int ceff, int nlive, double tol, double efr, int ndims, int nPar, int nClsPar, 
int maxModes, int updInt, double Ztol, char root[], int seed, int *pWrap, int fb, int resume, int outfile, 
int initMPI, double logZero, int maxiter, void (*LogLike)(double *, int *, int *, double *, TR_ringParameters *), 
void (*dumper)(int *, int *, int *, double **, double **, double **, double *, double *, double *, double *, TR_ringParameters *), 
TR_ringParameters *context);
